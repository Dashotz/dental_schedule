...
<div class="card">
    <div class="card-body">
        <form action="{{ route('availability.update', $availability) }}" method="POST" id="availabilityForm">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="type" name="type" required>
                    ...
                    <option value="weekly" {{ old('type', $availability->type) === 'weekly' ? 'selected' : '' }}>Weekly (Recurring)</option>
                    <option value="specific_date" {{ old('type', $availability->type) === 'specific_date' ? 'selected' : '' }}>Specific Date</option>
                    <option value="date_range" {{ old('type', $availability->type) === 'date_range' ? 'selected' : '' }}>Date Range</option>
                </select>
            ...
            <div class="mb-3" id="specificDateOptions" style="display: none;">
                ...
                <input type="date" class="form-control" id="specific_date" name="specific_date" value="{{ old('specific_date', $availability->specific_date?->format('Y-m-d')) }}" min="{{ date('Y-m-d') }}">
            </div>
            ...
            <div class="row mb-3" id="dateRangeOptions" style="display: none;">
                <div class="col-md-6">
                    ...
                    <input type="date" class="form-control" id="start_date" name="start_date" value="{{ old('start_date', $availability->start_date?->format('Y-m-d')) }}" min="{{ date('Y-m-d') }}">
                </div>
                <div class="col-md-6">
                    <label for="end_date" class="form-label">End Date <span class="text-danger">*</span></label>
                    <input type="date" class="form-control" id="end_date" name="end_date" value="{{ old('end_date', $availability->end_date?->format('Y-m-d')) }}" min="{{ date('Y-m-d') }}">
                </div>
            </div>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="time" class="form-control" id="start_time" name="start_time" value="{{ old('start_time', substr($availability->start_time, 0, 5)) }}" required>
                </div>
                <div class="col-md-6">
                    <label for="end_time" class="form-label">End Time <span class="text-danger">*</span></label>
                    <input type="time" class="form-control" id="end_time" name="end_time" value="{{ old('end_time', substr($availability->end_time, 0, 5)) }}" required>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="slot_duration" name="slot_duration" required>
                    ...
                    <option value="60" {{ old('slot_duration', $availability->slot_duration) == '60' ? 'selected' : '' }}>60 minutes</option>
                </select>
            </div>

            <div class="mb-3">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="is_available" name="is_available" value="1" {{ old('is_available', $availability->is_available) ? 'checked' : '' }}>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="notes" name="notes" rows="3" placeholder="Add any notes about this availability...">{{ old('notes', $availability->notes) }}</textarea>
            </div>
