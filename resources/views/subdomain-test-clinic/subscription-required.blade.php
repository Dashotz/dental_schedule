...
<body class="bg-gradient-to-br from-dental-teal-lighter to-white min-h-screen flex items-center justify-center p-4">
    <div class="max-w-2xl w-full">
        <div class="card-dental text-center p-8 md:p-12">
            <div class="w-24 h-24 mx-auto mb-6 bg-gradient-to-br from-dental-teal to-dental-teal-dark rounded-full flex items-center justify-center animate-pulse">
                <x-dental-icon name="{{ $reason === 'subscription_expired' ? 'credit-card' : 'exclamation-triangle' }}" class="w-16 h-16 text-white" />
            </div>
            ...
            <div class="p-6 rounded-xl mb-6 {{ $reason === 'subscription_expired' ? 'bg-yellow-50 border-2 border-yellow-300' : 'bg-red-50 border-2 border-red-300' }}">
                <h5 class="font-semibold text-lg mb-3 flex items-center justify-center gap-2">
                    <x-dental-icon name="{{ $reason === 'subscription_expired' ? 'info-circle' : 'x-circle' }}" class="w-5 h-5" />
                    {{ $subdomain->name }}
                </h5>
            ...
            <div class="bg-dental-teal-lighter rounded-xl p-6 mb-6">
                ...
                <p class="mb-2 text-gray-700">
                    ...
                    <strong>Email:</strong> 
                    <a href="mailto:{{ $subdomain->email }}" class="text-dental-teal hover:text-dental-teal-dark underline">
                        {{ $subdomain->email }}
                    </a>
                </p>
                @endif
                ...
                <p class="text-gray-700">
                    ...
                    <strong>Phone:</strong> 
                    <a href="tel:{{ $subdomain->phone }}" class="text-dental-teal hover:text-dental-teal-dark underline">
                        {{ $subdomain->phone }}
                    </a>
                </p>
                @endif
            ...
            <div class="mt-6">
                <a href="{{ url('/admin') }}" class="btn-dental inline-block">
                    <x-dental-icon name="box-arrow-right" class="w-5 h-5" /> Admin Panel
                </a>
            </div>
        </div>
    </div>
